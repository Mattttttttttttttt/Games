...
def callback(c):
    ...
    if 'c&n' in c.data and c.data[4] == '0':
        ...
        bot.send_document(chat_id = chat_id, caption = 'Ваш ход', document = open(f'/data/output_{chat_id}.png', 'rb'))
        df = cs.AI_move(df, chat_id)
        bot.send_document(chat_id = chat_id, caption = 'Ход бота', document = open(f'/data/output_{chat_id}.png', 'rb'))
